...
public class RenderFish extends Render
{
            ...
            float f, float f1)
    {
        ...
        if(entityfishhook.angler != null)
        {
            ...
            float f10 = entityfishhook.angler.getSwingProgress(f1);
            float f11 = MathHelper.sin(MathHelper.sqrt_float(f10) * 3.141593F);
            ...
            vec3d.rotateAroundY(f11 * 0.5F);
            vec3d.rotateAroundX(-f11 * 0.7F);
            double d5 = entityfishhook.angler.prevPosX + (entityfishhook.angler.posX - entityfishhook.angler.prevPosX) * (double)f1 + vec3d.xCoord;
            double d6 = entityfishhook.angler.prevPosY + (entityfishhook.angler.posY - entityfishhook.angler.prevPosY) * (double)f1 + vec3d.yCoord;
            double d7 = entityfishhook.angler.prevPosZ + (entityfishhook.angler.posZ - entityfishhook.angler.prevPosZ) * (double)f1 + vec3d.zCoord;
            if(renderManager.options.thirdPersonView > 0)
            {
                float f12 = ((entityfishhook.angler.prevRenderYawOffset + (entityfishhook.angler.renderYawOffset - entityfishhook.angler.prevRenderYawOffset) * f1) * 3.141593F) / 180F;
                double d9 = MathHelper.sin(f12);
                double d11 = MathHelper.cos(f12);
                d5 = (entityfishhook.angler.prevPosX + (entityfishhook.angler.posX - entityfishhook.angler.prevPosX) * (double)f1) - d11 * 0.34999999999999998D - d9 * 0.84999999999999998D;
                d6 = (entityfishhook.angler.prevPosY + (entityfishhook.angler.posY - entityfishhook.angler.prevPosY) * (double)f1) - 0.45000000000000001D;
                d7 = ((entityfishhook.angler.prevPosZ + (entityfishhook.angler.posZ - entityfishhook.angler.prevPosZ) * (double)f1) - d9 * 0.34999999999999998D) + d11 * 0.84999999999999998D;
            }
            double d8 = entityfishhook.prevPosX + (entityfishhook.posX - entityfishhook.prevPosX) * (double)f1;
            double d10 = entityfishhook.prevPosY + (entityfishhook.posY - entityfishhook.prevPosY) * (double)f1 + 0.25D;
