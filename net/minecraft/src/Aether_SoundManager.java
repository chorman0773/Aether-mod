...
//            SoundManager, SoundPool, GameSettings, CodecMus, 
//            EntityPlayerSP, mod_Aether, DimensionAether, SoundPoolEntry, 
//            EntityLiving, MathHelper

public class Aether_SoundManager extends SoundManager
{
    ...
    public SoundPool soundPoolAetherMusic;
    ...
    public Aether_SoundManager()
    {
        soundPoolSounds = new SoundPool();
        soundPoolStreaming = new SoundPool();
        soundPoolMusic = new SoundPool();
        soundPoolAetherMusic = new SoundPool();
        latestSoundID = 0;
        rand = new Random();
        ticksBeforeMusic = rand.nextInt(2000);
    }

    public void loadSoundSettings(GameSettings gamesettings)
    {
        loadSoundSettings(gamesettings, null);
    }

    public void loadSoundSettings(GameSettings gamesettings, SoundSystem soundsystem)
    {
        ...
        {
            tryToSetLibraryAndCodecs(soundsystem);
        }
        ...
        loadModAudio("minecraft/resources/mod/aethermusic", soundPoolAetherMusic);
        try
        {
    ...
    {
        tryToSetLibraryAndCodecs(null);
    }

    private void tryToSetLibraryAndCodecs(SoundSystem soundsystem)
    {
        try
        {
    ...
    public void playRandomMusicIfReady()
    {
        ...
        if(!sndSystem.playing("BgMusic") && !sndSystem.playing("streaming"))
        {
            ...
            if(mc.thePlayer.dimension == mod_Aether.dim.number)
            {
                soundpoolentry = soundPoolAetherMusic.getRandomSound();
            } else
            {
