...
//            AxisAlignedBB, DamageSource, EntityCreature, EntityPlayer, 
//            AetherBlocks, Block, ItemStack, AetherItems, 
//            MathHelper, BlockGrass, ModLoader, GuiIngame, 
//            ItemPickaxe, ItemTool, AetherAchievements, EffectRenderer, 
//            GameSettings
    ...
    implements IAetherBoss
{
    ...
    {
        for(int j = 0; j < 7 + rand.nextInt(3); j++)
        {
    ...
    public boolean attackEntityFrom(DamageSource damagesource, int i)
    {
        ...
        if(!(itemstack.getItem() instanceof ItemPickaxe) && (!(itemstack.getItem() instanceof ITextureProvider) || !(itemstack.getItem() instanceof ItemTool)))
        {
            chatItUp("Hmm. Perhaps I need to attack it with a Pickaxe?");
            return false;
        }
        boolean flag = super.attackEntityFrom(damagesource, Math.max(0, i));
        if(flag)
        {
            ...
            double d = Math.abs(posX - damagesource.getEntity().posX);
            double d2 = Math.abs(posZ - damagesource.getEntity().posZ);
            if(d > d2)
            {
                dennis = 1;
                rennis = 0;
                if(posX > damagesource.getEntity().posX)
                {
                    dennis = -1;
                }
            } else
            {
                rennis = 1;
                dennis = 0;
                if(posZ > damagesource.getEntity().posZ)
                {
                    rennis = -1;
                }
            }
    ...
    public void blockCrush(int i, int j, int k)
    {
        ...
        Block.blocksList[l].dropBlockAsItem(worldObj, i, j, k, i1, 1);
