...
//            IWorldAccess, RenderList, GLAllocation, OpenGlCapsChecker, 
//            RenderEngine, Tessellator, World, RenderManager, 
//            RenderBlocks, Block, GameSettings, BlockLeaves, 
//            WorldRenderer, Entity, MathHelper, EntitySorter, 
//            Profiler, TileEntityRenderer, EntityRenderer, EntityLiving, 
//            ICamera, RenderHelper, TileEntity, WorldProvider, 
//            Vec3D, EntityPlayerSP, RenderSorter, MovingObjectPosition, 
//            EntityPlayer, ItemStack, EnumMovingObjectType, AxisAlignedBB, 
//            GuiIngame, SoundManager, EntityHugeExplodeFX, EntityFX, 
//            EffectRenderer, EntityLargeExplodeFX, EntityBubbleFX, EntitySuspendFX, 
//            EntityAuraFX, EntityCritFX, EntitySmokeFX, EntitySpellParticleFX, 
    ...
    implements IWorldAccess
{
    ...
    public void renderSky(float f)
    {
        ...
        if(af != null)
        {
            ...
            GL11.glPushMatrix();
            GL11.glRotatef(90F, 1.0F, 0.0F, 0.0F);
            GL11.glRotatef(MathHelper.sin(worldObj.getCelestialAngleRadians(f)) < 0.0F ? 180F : 0.0F, 0.0F, 0.0F, 1.0F);
            GL11.glRotatef(90F, 0.0F, 0.0F, 1.0F);
            float f7 = af[0];
            float f9 = af[1];
            float f11 = af[2];
            if(mc.gameSettings.anaglyph)
            {
                float f13 = (f7 * 30F + f9 * 59F + f11 * 11F) / 100F;
                float f15 = (f7 * 30F + f9 * 70F) / 100F;
                float f17 = (f7 * 30F + f11 * 70F) / 100F;
                f7 = f13;
                f9 = f15;
                f11 = f17;
            }
            tessellator1.startDrawing(6);
            tessellator1.setColorRGBA_F(f7, f9, f11, af[3]);
            tessellator1.addVertex(0.0D, 100D, 0.0D);
            int j = 16;
            tessellator1.setColorRGBA_F(af[0], af[1], af[2], 0.0F);
            for(int k = 0; k <= j; k++)
            {
                float f18 = ((float)k * 3.141593F * 2.0F) / (float)j;
                float f19 = MathHelper.sin(f18);
                float f20 = MathHelper.cos(f18);
                tessellator1.addVertex(f19 * 120F, f20 * 120F, -f20 * 40F * af[3]);
            }
        ...
        GL11.glPushMatrix();
        float f8 = 1.0F - worldObj.getRainStrength(f);
        float f10 = 0.0F;
        float f12 = 0.0F;
        float f14 = 0.0F;
        ...
        int l = worldObj.func_40475_d(f);
        int i1 = l % 4;
        int j1 = (l / 4) % 2;
        float f21 = (float)(i1 + 0) / 4F;
        float f22 = (float)(j1 + 0) / 2.0F;
        float f23 = (float)(i1 + 1) / 4F;
        float f24 = (float)(j1 + 1) / 2.0F;
        tessellator1.startDrawingQuads();
        tessellator1.addVertexWithUV(-f16, -100D, f16, f23, f24);
        tessellator1.addVertexWithUV(f16, -100D, f16, f21, f24);
        tessellator1.addVertexWithUV(f16, -100D, -f16, f21, f22);
        tessellator1.addVertexWithUV(-f16, -100D, -f16, f23, f22);
        tessellator1.draw();
        GL11.glDisable(3553 /*GL_TEXTURE_2D*/);
        float f25 = worldObj.getStarBrightness(f) * f8;
        if(f25 > 0.0F)
        {
            GL11.glColor4f(f25, f25, f25, f25);
        ...
        double d = 1.0D;
        if(mc.theWorld.worldProvider.worldType != 2)
        {
            d = mc.thePlayer.getPosition(f).yCoord - 63D;
        }
        if(d < 0.0D)
        {
            GL11.glPushMatrix();
            ...
            GL11.glPopMatrix();
            float f26 = 1.0F;
            float f27 = -(float)(d + 65D);
            float f28 = -f26;
            float f29 = f27;
            tessellator1.startDrawingQuads();
            tessellator1.setColorRGBA_I(0, 255);
            tessellator1.addVertex(-f26, f29, f26);
            tessellator1.addVertex(f26, f29, f26);
            tessellator1.addVertex(f26, f28, f26);
            tessellator1.addVertex(-f26, f28, f26);
            tessellator1.addVertex(-f26, f28, -f26);
            tessellator1.addVertex(f26, f28, -f26);
            tessellator1.addVertex(f26, f29, -f26);
            ...
            float f, float f1)
    {
        ...
        if(mc.renderViewEntity != null && mc.renderViewEntity.getDistanceSq(d, d1, d2) < (double)(f2 * f2))
        {
    ...
    public void playAuxSFX(EntityPlayer entityplayer, int i, int j, int k, int l, int i1)
    {
        ...
        case 2000: 
            ...
            for(int l1 = 0; l1 < 10; l1++)
            {
                ...
                double d6 = d + (double)j1 * 0.01D + (random.nextDouble() - 0.5D) * (double)k1 * 0.5D;
                double d8 = d1 + (random.nextDouble() - 0.5D) * 0.5D;
                double d9 = d2 + (double)k1 * 0.01D + (random.nextDouble() - 0.5D) * (double)j1 * 0.5D;
                double d12 = (double)j1 * d4 + random.nextGaussian() * 0.01D;
                double d14 = -0.029999999999999999D + random.nextGaussian() * 0.01D;
                double d16 = (double)k1 * d4 + random.nextGaussian() * 0.01D;
        ...
        case 2003: 
            double d3 = (double)j + 0.5D;
            double d5 = k;
            double d7 = (double)l + 0.5D;
        ...
        case 2002: 
            ...
            int k2 = Item.potion.getColorFromDamage(i1);
            float f = (float)(k2 >> 16 & 0xff) / 255F;
            float f1 = (float)(k2 >> 8 & 0xff) / 255F;
            float f2 = (float)(k2 >> 0 & 0xff) / 255F;
            String s2 = "spell";
        ...
        case 2004: 
            ...
            {
                double d18 = (double)j + 0.5D + ((double)worldObj.rand.nextFloat() - 0.5D) * 2D;
                double d20 = (double)k + 0.5D + ((double)worldObj.rand.nextFloat() - 0.5D) * 2D;
                double d22 = (double)l + 0.5D + ((double)worldObj.rand.nextFloat() - 0.5D) * 2D;
