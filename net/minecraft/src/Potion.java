...
public class Potion
{
    ...
    public void performEffect(EntityLiving entityliving, int i)
    {
        ...
        if(id == heal.id && !entityliving.isEntityUndead() || id == potionHealDamage.id && entityliving.isEntityUndead())
        {
            entityliving.heal(6 << i);
        } else
        if(id == potionHealDamage.id && !entityliving.isEntityUndead() || id == heal.id && entityliving.isEntityUndead())
        {
    ...
    {
        if(id == heal.id && !entityliving1.isEntityUndead() || id == potionHealDamage.id && entityliving1.isEntityUndead())
        {
            int j = (int)(d * (double)(6 << i) + 0.5D);
            entityliving1.heal(j);
        } else
        if(id == potionHealDamage.id && !entityliving1.isEntityUndead() || id == heal.id && entityliving1.isEntityUndead())
        {
