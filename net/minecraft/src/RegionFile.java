    ...
    protected synchronized void write(int i, int j, byte abyte0[], int k)
    {
        try
        {
            ...
            } else
            {
                ...
                int i2 = sectorFree.indexOf(Boolean.valueOf(true));
                int j2 = 0;
                if(i2 != -1)
                {
                    int k2 = i2;
                    do
                    {
                        if(k2 >= sectorFree.size())
                        {
                            break;
                        }
                        if(j2 != 0)
                        {
                            if(((Boolean)sectorFree.get(k2)).booleanValue())
                            {
                                j2++;
                            } else
                            {
                ...
                if(j2 >= k1)
                {
                    ...
                    int l2 = i2;
                    setOffset(i, j, l2 << 8 | k1);
                    for(int j3 = 0; j3 < k1; j3++)
                    {
                        sectorFree.set(l2 + j3, Boolean.valueOf(false));
                    }
