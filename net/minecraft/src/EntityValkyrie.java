...
//            DamageSource, AetherBlocks, AetherAchievements
    ...
    implements IAetherBoss
{
    ...
    public void onUpdate()
    {
        lastMotionY = motionY;
        super.onUpdate();
        if(!onGround && entityToAttack != null && lastMotionY >= 0.0D && motionY < 0.0D && getDistanceToEntity(entityToAttack) <= 16F && canEntityBeSeen(entityToAttack))
        {
            double d = entityToAttack.posX - posX;
            double d1 = entityToAttack.posZ - posZ;
        ...
        moveSpeed = entityToAttack != null ? 1.0F : 0.5F;
        if(worldObj.difficultySetting <= 0 && (entityToAttack != null || angerLevel > 0))
        {
            angerLevel = 0;
            entityToAttack = null;
        }
    ...
    public boolean interact(EntityPlayer entityplayer)
    {
        ...
        if(!duel)
        {
            ...
            if(itemstack1 != null && itemstack1.itemID == AetherItems.VictoryMedal.shiftedIndex && itemstack1.stackSize >= 10)
            {
                ...
                {
                    itemstack1.onItemDestroyedByUse(entityplayer);
    ...
    public void makeHomeShot(int i, EntityLiving entityliving)
    {
        for(int j = 0; j < i; j++)
        {
            ...
            worldObj.spawnEntityInWorld(entityhomeshot);
        }
    ...
    public void updateEntityActionState()
    {
        ...
        if(teleTimer >= 450)
        {
            ...
            {
                if(boss && onGround && rand.nextInt(2) == 0 && entityToAttack != null && (entityToAttack instanceof EntityLiving))
                {
                    makeHomeShot(1, (EntityLiving)entityToAttack);
                    teleTimer = -100;
                } else
                {
                    teleport(entityToAttack.posX, entityToAttack.posY, entityToAttack.posZ, 7);
                }
            } else
            if(!onGround || boss)
            {
        ...
        if(teleTimer % 5 == 0 && entityToAttack != null && !canEntityBeSeen(entityToAttack))
        {
            teleTimer += 100;
        }
    ...
    {
        if((damagesource.getEntity() instanceof EntityPlayer) && worldObj.difficultySetting > 0)
        {
    ...
    protected void attackEntity(Entity entity, float f)
    {
        if(attackTime <= 0 && f < 2.75F && entity.boundingBox.maxY > boundingBox.minY && entity.boundingBox.minY < boundingBox.maxY)
        {
            ...
            if(entity != null && entityToAttack != null && entity == entityToAttack && (entity instanceof EntityLiving))
            {
