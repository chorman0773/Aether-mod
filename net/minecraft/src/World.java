...
//            IBlockAccess, SAPI, WorldProvider, WorldInfo, 
//            MapStorage, ISaveHandler, WorldSettings, DimensionBase, 
//            ThreadedChunkLoader, ChunkProvider, WorldChunkManager, ChunkPosition, 
//            EntityPlayer, NBTTagCompound, ChunkProviderLoadOrGenerate, MathHelper, 
//            IChunkProvider, IProgressUpdate, Chunk, Block, 
//            IBlockSet, Material, IWorldAccess, EnumSkyBlock, 
//            Vec3D, Entity, AxisAlignedBB, BiomeGenBase, 
...
//            PlayerCapabilities, ChunkCoordinates, WeightedRandom, SpawnListEntry, 
    ...
    implements IBlockAccess
{
    ...
    public World(ISaveHandler isavehandler, String s, WorldProvider worldprovider, WorldSettings worldsettings)
    {
        ...
        worldHeight = 1 << field_35473_a;
        field_35469_d = worldHeight - 1;
        field_35470_e = worldHeight / 2 - 1;
    ...
    public World(ISaveHandler isavehandler, String s, WorldSettings worldsettings, WorldProvider worldprovider)
    {
        ...
        } else
        {
            if(worldInfo != null)
            {
                i = worldInfo.getDimension();
            }
            worldProvider = DimensionBase.getWorldProvider(i);
        }
        boolean flag = false;
        if(worldInfo == null)
        {
        ...
        worldProvider.registerWorld(this);
        if(DimensionBase.providerMap.containsKey(Integer.valueOf(i)))
        {
            DimensionBase dimensionbase = (DimensionBase)DimensionBase.dimMap.get(Integer.valueOf(i));
            java.io.File file = dimensionbase.GetDimensionSaveDir(saveHandler);
            ThreadedChunkLoader threadedchunkloader = new ThreadedChunkLoader(file);
            chunkProvider = new ChunkProvider(this, threadedchunkloader, worldProvider.getChunkProvider());
        } else
        {
            chunkProvider = getChunkProvider();
        }
    ...
    public boolean isAirBlock(int i, int j, int k)
    {
        ...
        {
            return Block.blocksList[l].isAirBlock(this, i, j, k);
        }
    }
    ...
    public boolean setBlockAndMetadata(int i, int j, int k, int l, int i1)
    {
        ...
        int j1 = l;
        IBlockSet iblockset = SAPI.BlockSet.getIntercept(this, i, j, k, j1);
        if(iblockset != null)
        {
            l = iblockset.iBlockGet(this, i, j, k, j1);
        }
        ...
        if(iblockset != null)
        {
            iblockset.iAfterIntercept(this, i, j, k, j1);
        }
        return flag;
    }
    ...
    public boolean setBlock(int i, int j, int k, int l)
    {
        ...
        int i1 = l;
        IBlockSet iblockset = SAPI.BlockSet.getIntercept(this, i, j, k, i1);
        if(iblockset != null)
        {
            l = iblockset.iBlockGet(this, i, j, k, i1);
        }
        Chunk chunk = getChunkFromChunkCoords(i >> 4, k >> 4);
        ...
        if(iblockset != null)
        {
            iblockset.iAfterIntercept(this, i, j, k, i1);
        }
        return flag;
    }
...
label1:
    ...
    protected void updateBlocksAndPlayCaveSounds()
    {
        ...
        for(Iterator iterator = positionsToUpdate.iterator(); iterator.hasNext(); Profiler.endSection())
        {
            ...
            if(soundCounter == 0)
            {
                ...
                int i3 = j2 & 0xf;
                int l3 = j2 >> 8 & 0xf;
                int k4 = j2 >> 16 & field_35469_d;
                int j5 = chunk.getBlockID(i3, k4, l3);
                i3 += j1;
                l3 += l1;
                if(j5 == 0 && getFullBlockLightValue(i3, k4, l3) <= rand.nextInt(8) && getSavedLightValue(EnumSkyBlock.Sky, i3, k4, l3) <= 0)
                {
                    EntityPlayer entityplayer1 = getClosestPlayer((double)i3 + 0.5D, (double)k4 + 0.5D, (double)l3 + 0.5D, 8D);
                    if(entityplayer1 != null && entityplayer1.getDistanceSq((double)i3 + 0.5D, (double)k4 + 0.5D, (double)l3 + 0.5D) > 4D)
                    {
                        playSoundEffect((double)i3 + 0.5D, (double)k4 + 0.5D, (double)l3 + 0.5D, "ambient.cave.cave", 0.7F, 0.8F + rand.nextFloat() * 0.2F);
            ...
            if(rand.nextInt(0x186a0) == 0 && isRaining() && getIsThundering())
            {
                ...
                int j3 = j1 + (k2 & 0xf);
                int i4 = l1 + (k2 >> 8 & 0xf);
                int l4 = func_35461_e(j3, i4);
                if(canLightningStrikeAt(j3, l4, i4))
                {
                    addWeatherEffect(new EntityLightningBolt(this, j3, l4, i4));
                    lastLightningBolt = 2;
                }
            }
            ...
            int k3 = l2 & 0xf;
            int j4 = l2 >> 8 & 0xf;
            int i5 = func_35461_e(k3 + j1, j4 + l1);
            if(func_40481_q(k3 + j1, i5 - 1, j4 + l1))
            {
                setBlockWithNotify(k3 + j1, i5 - 1, j4 + l1, Block.ice.blockID);
            }
            if(isRaining() && canSnowAt(k3 + j1, i5, j4 + l1))
            {
                setBlockWithNotify(k3 + j1, i5, j4 + l1, Block.snow.blockID);
            }
            ...
            for(int k5 = 0; k5 < 20; k5++)
            {
                ...
                int i6 = l5 & 0xf;
                int j6 = l5 >> 8 & 0xf;
                int k6 = l5 >> 16 & field_35469_d;
                int l6 = chunk.blocks[i6 << field_35471_b | j6 << field_35473_a | k6] & 0xff;
                k++;
                if(Block.tickOnLoad[l6])
                {
                    j++;
                    Block.blocksList[l6].updateTick(this, i6 + j1, k6, j6 + l1, rand);
                }
            }
    ...
    {
        int k1 = i1 != 0 ? Block.blocksList[i1].getLightValue(this, j, k, l) : 0;
    ...
    public void updateLightByType(EnumSkyBlock enumskyblock, int i, int j, int k)
    {
        ...
        int k1 = 0;
        int l1 = j1;
        int i2 = getBlockId(i, j, k);
        int j2 = Block.lightOpacity[i2];
        if(j2 == 0)
        {
            j2 = 1;
        }
        ...
        {
            k2 = computeSkyLightValue(l1, i, j, k, i2, j2);
        } else
        {
            k2 = computeBlockLightValue(l1, i, j, k, i2, j2);
        }
        k1 = k2;
        if(k1 > j1)
        {
        ...
        if(k1 < j1)
        {
            ...
            do
            {
                ...
                int l2 = lightUpdateBlockList[l++];
                int j3 = ((l2 & 0x3f) - 32) + i;
                int l3 = ((l2 >> 6 & 0x3f) - 32) + j;
                int j4 = ((l2 >> 12 & 0x3f) - 32) + k;
                int l4 = l2 >> 18 & 0xf;
                int j5 = getSavedLightValue(enumskyblock, j3, l3, j4);
                if(j5 == l4)
                {
                    setLightValue(enumskyblock, j3, l3, j4, 0);
                    if(l4 > 0)
                    {
                        int l5 = j3 - i;
                        int j6 = l3 - j;
                        ...
                        if(l5 + j6 + l6 < 17)
                        {
                            ...
                            {
                                int l7 = (j7 % 2) * 2 - 1;
                                int j8 = j3 + (((j7 / 2) % 3) / 2) * l7;
                                int k8 = l3 + (((j7 / 2 + 1) % 3) / 2) * l7;
                                int l8 = j4 + (((j7 / 2 + 2) % 3) / 2) * l7;
                                int i9 = getSavedLightValue(enumskyblock, j8, k8, l8);
                                int j9 = Block.lightOpacity[getBlockId(j8, k8, l8)];
                                if(j9 == 0)
                                {
                                    j9 = 1;
                                }
                                if(i9 == l4 - j9)
                                {
                                    lightUpdateBlockList[i1++] = (j8 - i) + 32 + ((k8 - j) + 32 << 6) + ((l8 - k) + 32 << 12) + (l4 - j9 << 18);
                                }
                                j7++;
                            }
                        }
        ...
        do
        {
            ...
            int i3 = lightUpdateBlockList[l++];
            int k3 = ((i3 & 0x3f) - 32) + i;
            int i4 = ((i3 >> 6 & 0x3f) - 32) + j;
            int k4 = ((i3 >> 12 & 0x3f) - 32) + k;
            int i5 = getSavedLightValue(enumskyblock, k3, i4, k4);
            int k5 = getBlockId(k3, i4, k4);
            int i6 = Block.lightOpacity[k5];
            if(i6 == 0)
            {
                i6 = 1;
            }
            ...
            if(enumskyblock == EnumSkyBlock.Sky)
            {
                k6 = computeSkyLightValue(i5, k3, i4, k4, k5, i6);
            } else
            {
                k6 = computeBlockLightValue(i5, k3, i4, k4, k5, i6);
            }
            if(k6 != i5)
            {
                setLightValue(enumskyblock, k3, i4, k4, k6);
                if(k6 > i5)
                {
                    int i7 = k3 - i;
                    int k7 = i4 - j;
                    ...
                    if(i7 + k7 + i8 < 17 && i1 < lightUpdateBlockList.length - 6)
                    {
                        if(getSavedLightValue(enumskyblock, k3 - 1, i4, k4) < k6)
                        {
                            lightUpdateBlockList[i1++] = (k3 - 1 - i) + 32 + ((i4 - j) + 32 << 6) + ((k4 - k) + 32 << 12);
                        }
                        if(getSavedLightValue(enumskyblock, k3 + 1, i4, k4) < k6)
                        {
                            lightUpdateBlockList[i1++] = ((k3 + 1) - i) + 32 + ((i4 - j) + 32 << 6) + ((k4 - k) + 32 << 12);
                        }
                        if(getSavedLightValue(enumskyblock, k3, i4 - 1, k4) < k6)
                        {
                            lightUpdateBlockList[i1++] = (k3 - i) + 32 + ((i4 - 1 - j) + 32 << 6) + ((k4 - k) + 32 << 12);
                        }
                        if(getSavedLightValue(enumskyblock, k3, i4 + 1, k4) < k6)
                        {
                            lightUpdateBlockList[i1++] = (k3 - i) + 32 + (((i4 + 1) - j) + 32 << 6) + ((k4 - k) + 32 << 12);
                        }
                        if(getSavedLightValue(enumskyblock, k3, i4, k4 - 1) < k6)
                        {
                            lightUpdateBlockList[i1++] = (k3 - i) + 32 + ((i4 - j) + 32 << 6) + ((k4 - 1 - k) + 32 << 12);
                        }
                        if(getSavedLightValue(enumskyblock, k3, i4, k4 + 1) < k6)
                        {
                            lightUpdateBlockList[i1++] = (k3 - i) + 32 + ((i4 - j) + 32 << 6) + (((k4 + 1) - k) + 32 << 12);
