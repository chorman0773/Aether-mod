...
public class EntityDragon extends EntityDragonBase
{
    ...
    public void onLivingUpdate()
    {
        ...
        } else
        {
            ...
            float f15 = (float)(vec3d1.dotProduct(vec3d) + 0.5D) / 1.5F;
            if(f15 < 0.0F)
            {
                f15 = 0.0F;
            }
            randomYawVelocity *= 0.8F;
            float f17 = MathHelper.sqrt_double(motionX * motionX + motionZ * motionZ) * 1.0F + 1.0F;
            double d14 = Math.sqrt(motionX * motionX + motionZ * motionZ) * 1.0D + 1.0D;
            if(d14 > 40D)
            {
                d14 = 40D;
            }
            randomYawVelocity += d11 * (0.69999998807907104D / d14 / (double)f17);
            rotationYaw += randomYawVelocity * 0.1F;
            float f20 = (float)(2D / (d14 + 1.0D));
            float f21 = 0.06F;
            moveFlying(0.0F, -1F, f21 * (f15 * f20 + (1.0F - f20)));
            if(field_40161_az)
            {
        ...
        for(int j = 0; j < 3; j++)
        {
            ...
            float f16 = MathHelper.cos(f13);
            float f18 = 1.5F;
            float f19 = (float)(j + 1) * 2.0F;
            dragonpart.onUpdate();
            dragonpart.setLocationAndAngles(posX - (double)((f8 * f18 + f14 * f19) * f5), ((posY + (ad2[1] - ad[1]) * 1.0D) - (double)((f19 + f18) * f6)) + 1.5D, posZ + (double)((f9 * f18 + f16 * f19) * f5), 0.0F, 0.0F);
        }
    ...
    protected void func_40120_m_()
    {
        ...
        if(!worldObj.multiplayerWorld && field_40178_aA > 150 && field_40178_aA % 5 == 0)
        {
            for(int i = 1000; i > 0;)
            {
                int l = EntityXPOrb.getXPSplit(i);
                i -= l;
        ...
        if(field_40178_aA == 200)
        {
            for(int j = 10000; j > 0;)
            {
                int i1 = EntityXPOrb.getXPSplit(j);
                j -= i1;
                worldObj.spawnEntityInWorld(new EntityXPOrb(worldObj, posX, posY, posZ, i1));
            }

            int k = (5 + rand.nextInt(2) * 2) - 1;
