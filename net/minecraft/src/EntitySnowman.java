...
public class EntitySnowman extends EntitySnowmanBase
{
    ...
    public void onLivingUpdate()
    {
        ...
        if(worldObj.getWorldChunkManager().func_35554_b(i, j, k) > 1.0F)
        {
            attackEntityFrom(DamageSource.onFire, 1);
        }
        for(int l = 0; l < 4; l++)
        {
            int i1 = MathHelper.floor_double(posX + (double)((float)((l % 2) * 2 - 1) * 0.25F));
            int j1 = MathHelper.floor_double(posY);
            int k1 = MathHelper.floor_double(posZ + (double)((float)(((l / 2) % 2) * 2 - 1) * 0.25F));
            if(worldObj.getBlockId(i1, j1, k1) == 0 && worldObj.getWorldChunkManager().func_35554_b(i1, j1, k1) < 0.8F && Block.snow.canPlaceBlockAt(worldObj, i1, j1, k1))
            {
                worldObj.setBlockWithNotify(i1, j1, k1, Block.snow.blockID);
            }
        }
