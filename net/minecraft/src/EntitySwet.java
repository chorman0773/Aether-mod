...
//            MathHelper, DamageSource, EntityLiving, EntityPlayer, 
//            NBTTagCompound, EntityMob, ItemStack, AetherItems, 
//            Item, mod_Aether
...
public class EntitySwet extends EntityAetherAnimal
{
    ...
    {
        if(hops == 3 && entityliving == null && health > 1)
        {
            health = 1;
        }
        boolean flag = super.attackEntityFrom(DamageSource.causeMobDamage(entityliving), i);
        if(flag && riddenByEntity != null && (riddenByEntity instanceof EntityLiving))
        {
            if(entityliving != null && riddenByEntity == entityliving)
            {
                if(rand.nextInt(3) == 0)
                {
                    kickoff = true;
            ...
            {
                riddenByEntity.attackEntityFrom(DamageSource.causeMobDamage(this), i);
                if(health <= 0)
                {
                    kickoff = true;
                }
        ...
        if(flag && (entityliving instanceof EntityLiving))
        {
            EntityLiving entityliving1 = entityliving;
            if(entityliving1.health > 0 && (riddenByEntity == null || entityliving1 != riddenByEntity))
            {
                entityToAttack = entityliving;
                faceEntity(entityliving, 180F, 180F);
                kickoff = true;
            }
        }
        if(friendly && (entityToAttack instanceof EntityPlayer))
        {
            entityToAttack = null;
        }
        return flag;
    ...
    public void updateEntityActionState()
    {
        ...
        if(entityToAttack != null && riddenByEntity == null && health > 0)
        {
        ...
        } else
        {
            ...
            if(entityToAttack == null && riddenByEntity == null)
            {
            ...
            if(entityToAttack != null && riddenByEntity == null)
            {
                if(getDistanceToEntity(entityToAttack) <= 9F)
                {
                    if(onGround && canEntityBeSeen(entityToAttack))
                    {
                        splotch();
                        flutter = 10;
                        isJumping = true;
    ...
    {
        if(hops == 0 && riddenByEntity == null && entityToAttack != null && entity != null && entity == entityToAttack && (entity.ridingEntity == null || !(entity.ridingEntity instanceof EntitySwet)))
        {
    ...
    {
        ItemStack itemstack = new ItemStack(textureNum != 1 ? AetherItems.SwetBall.shiftedIndex : AetherItems.SwetBall.shiftedIndex, 6, textureNum != 1 ? 0 : 1);
