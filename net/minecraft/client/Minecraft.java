    ...
    implements Runnable
{
    ...
    private void displayDebugInfo(long l)
    {
        ...
        for(int j3 = 0; j3 < list.size(); j3++)
        {
            ...
            int k3 = MathHelper.floor_double(profilerresult1.field_40704_a / 4D) + 1;
            tessellator.startDrawing(6);
            tessellator.setColorOpaque_I(profilerresult1.func_40700_a());
            tessellator.addVertex(i2, k2, 0.0D);
            for(int j4 = k3; j4 >= 0; j4--)
            {
                float f = (float)(((d + (profilerresult1.field_40704_a * (double)j4) / (double)k3) * 3.1415927410125732D * 2D) / 100D);
            ...
            {
                float f1 = (float)(((d + (profilerresult1.field_40704_a * (double)k4) / (double)k3) * 3.1415927410125732D * 2D) / 100D);
        ...
        int i4 = 0xffffff;
        fontRenderer.drawStringWithShadow(s, i2 - j1, k2 - j1 / 2 - 16, i4);
        fontRenderer.drawStringWithShadow(s = (new StringBuilder()).append(decimalformat.format(profilerresult.field_40702_b)).append("%").toString(), (i2 + j1) - fontRenderer.getStringWidth(s), k2 - j1 / 2 - 16, i4);
        for(int i5 = 0; i5 < list.size(); i5++)
        {
            ProfilerResult profilerresult2 = (ProfilerResult)list.get(i5);
            String s1 = "";
            if(!profilerresult2.field_40703_c.equals("unspecified"))
            {
                s1 = (new StringBuilder()).append(s1).append("[").append(i5 + 1).append("] ").toString();
            } else
            {
                s1 = (new StringBuilder()).append(s1).append("[?] ").toString();
            }
            s1 = (new StringBuilder()).append(s1).append(profilerresult2.field_40703_c).toString();
            fontRenderer.drawStringWithShadow(s1, i2 - j1, k2 + j1 / 2 + i5 * 8 + 20, profilerresult2.func_40700_a());
            fontRenderer.drawStringWithShadow(s1 = (new StringBuilder()).append(decimalformat.format(profilerresult2.field_40704_a)).append("%").toString(), (i2 + j1) - 50 - fontRenderer.getStringWidth(s1), k2 + j1 / 2 + i5 * 8 + 20, profilerresult2.func_40700_a());
            fontRenderer.drawStringWithShadow(s1 = (new StringBuilder()).append(decimalformat.format(profilerresult2.field_40702_b)).append("%").toString(), (i2 + j1) - fontRenderer.getStringWidth(s1), k2 + j1 / 2 + i5 * 8 + 20, profilerresult2.func_40700_a());
        }
    ...
    private void clickMouse(int i)
    {
        ...
        if(objectMouseOver.typeOfHit == EnumMovingObjectType.TILE)
        {
            ...
            } else
            {
                ...
                int j1 = itemstack2 != null ? itemstack2.stackSize : 0;
